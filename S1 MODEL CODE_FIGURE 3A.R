...
parameters_1 <- c(
  ...
  beta_2 = 0, # underlying rate of transmission for variant 2 = p * c
  ...
  kappa = 0, # reduction in transmission due to behaviour change
  start_kappa = 0, # START OF BEHAVIOURAL MEDIATION
  start_I2 = 500 # START OF VARIANT 2 TRANSMISSION
)
...
ggplot(out_long, aes(x = time, y = value, color = variable)) +
  ...
  scale_color_manual(values = c("I1" = "black", "I2" = "blue"),
                     name = "Legend", labels = c("Variant 1", "Variant 2")) +
  scale_y_continuous(name = "Total Infections (Prevalence)", limits = c(0, 625)) +
  labs(x = "Time", title = "No Variant 2, No Behavioural Mediation") +
  theme_minimal() +
  theme(
...
# SAVING THE PLOT
ggsave("variant_prevalence_plot_1.png", width = 20, height = 6, dpi = 300)
print("Plot has been saved as variant_prevalence_plot_1.png")