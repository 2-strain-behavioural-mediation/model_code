...
ggplot(out_long, aes(x = time, y = value, color = variable)) +
  ...
  scale_y_continuous(name = "Infected Individuals",
                     sec.axis = sec_axis(~ . / max(out_long$value[out_long$variable != "policy_kappa_threshold"]), 
                                         name = "Policy Kappa Threshold",
                                         breaks = seq(0, 1, 0.2))) +
  ...
  geom_line(data = filter(out_long, variable == "policy_kappa_threshold"), 
            aes(y = value * max(out_long$value[out_long$variable != "policy_kappa_threshold"])), 
